...
const productos = document.querySelector('[data-unstableGamesCategoria]');
const productos2 = document.querySelector('[data-devirCategoria]');
...
const render = async () => {
    try {
        ...
        if (productos) {
            productos.innerHTML = '';
            allProducts.filter(product => product.categoria === 'unstableGames').forEach(elemento => {
                productos.appendChild(nuevoProducto(elemento.name, elemento.price, elemento.imageUrl, elemento.id));
            });
        }
        if (productos2) {
            productos2.innerHTML = '';
            allProducts.filter(product => product.categoria === 'Devir').forEach(elemento => {
